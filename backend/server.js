...
//Fetch User Details
server.get('/api/userdetails', async (rqst, rspn) => {
  try {
...
server.post('/api/search', async (rqst, rspn) => {
  try {
    ...
    const [items] = await db.execute('SELECT * FROM items WHERE item_name LIKE ?', [`%${keyword}%`]);
...
//Buy item
server.post('/api/buy', async (rqst, rspn) => {
